...
<div class="notif-center-header">
    ...
    <div class="notif-header-info">
        ...
        <p>Monitor system-wide alerts, security notifications, and administrative updates</p>
    </div>
</div>
...
<div class="notif-stats-row">
    <div class="notif-stat-card">
        ...
        <div class="nsc-info"><h4 id="statTotal"><?php echo $total_all; ?></h4><p>Total Notifications</p></div>
    </div>
    <div class="notif-stat-card">
...
<div class="notif-toolbar">
    ...
    <div class="notif-actions">
        ...
        <button class="notif-action-btn" onclick="bulkAction('mark_all_read')"><i class="fas fa-check-double"></i> Mark All Read</button>
        <?php endif; ?>
        <?php if ($total_read > 0): ?>
        <button class="notif-action-btn danger" onclick="bulkAction('delete_all_read')"><i class="fas fa-trash-alt"></i> Clear Read</button>
        <?php endif; ?>
    </div>
</div>
...
<?php if (empty($notifications)): ?>
    <div class="notif-empty-state">
        ...
        <p><?php echo ($filter !== 'all' || $search !== '') ? 'Try changing your filter or search terms.' : "You're all caught up! New notifications will appear here when actions occur in the system."; ?></p>
    </div>
<?php else: ?>
    ...
    <div class="notif-pagination">
        <?php if ($page > 1): ?>
            <a href="<?php echo $base_url; ?>?filter=<?php echo $filter; ?>&search=<?php echo urlencode($search); ?>&page=<?php echo $page - 1; ?>"><i class="fas fa-chevron-left"></i></a>
        <?php else: ?><span class="disabled"><i class="fas fa-chevron-left"></i></span><?php endif; ?>
        <?php $start_page = max(1, $page - 3); $end_page = min($total_pages, $start_page + 6);
        if ($end_page - $start_page < 6) $start_page = max(1, $end_page - 6);
        for ($p = $start_page; $p <= $end_page; $p++): ?>
            <?php if ($p === $page): ?><span class="current"><?php echo $p; ?></span>
            <?php else: ?><a href="<?php echo $base_url; ?>?filter=<?php echo $filter; ?>&search=<?php echo urlencode($search); ?>&page=<?php echo $p; ?>"><?php echo $p; ?></a><?php endif; ?>
        <?php endfor; ?>
        <?php if ($page < $total_pages): ?>
            <a href="<?php echo $base_url; ?>?filter=<?php echo $filter; ?>&search=<?php echo urlencode($search); ?>&page=<?php echo $page + 1; ?>"><i class="fas fa-chevron-right"></i></a>
...
function navigateNotif(id, link, isUnread) {
    if (isUnread) { const fd = new FormData(); fd.append('notification_id', id); fd.append('action', 'mark_read'); fetch(AJAX_URL, { method: 'POST', body: fd }); }
...
function updateStats() {
    const cards = document.querySelectorAll('.notif-card'); let u = 0;
    cards.forEach(c => { if (c.dataset.read === '0') u++; });
    document.getElementById('statTotal').textContent = cards.length;
    document.getElementById('statUnread').textContent = u;
    document.getElementById('statRead').textContent = cards.length - u;
    const badge = document.querySelector('.notification-badge');
    if (badge) { badge.textContent = u > 9 ? '9+' : u; badge.style.display = u === 0 ? 'none' : ''; }
}
</script>
